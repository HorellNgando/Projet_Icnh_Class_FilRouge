...
const MedicationList = () => {
    ...
    useEffect(() => {
        const filtered = (medications || []).filter(medication => 
            ...
            medication.stock?.toString().includes(searchTerm)
        );
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">Médicaments</h2>
                    <div className="flex space-x-4">
                        ...
                        <button
                            onClick={() => navigate('/dashboard/admin/medications/create')}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {currentMedications.map((medication) => (
                                <tr 
                                    ...
                                    onClick={() => navigate(`/dashboard/admin/medications/${medication.id}`)}
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStockStatus(medication.stock)}`}>
                                            {medication.stock}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <div>
                                <p className="text-sm text-gray-700">
                                    Affichage de <span className="font-medium">{indexOfFirstMedication + 1}</span> à{' '}
                                    <span className="font-medium">
                                        {Math.min(indexOfLastMedication, filteredMedications.length)}
                                    </span>{' '}
                                    sur <span className="font-medium">{filteredMedications.length}</span> résultats
                                </p>
                            </div>
                            <div>
